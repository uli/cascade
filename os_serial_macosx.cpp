...
{
  int sh;
  char *sysfs_dir = "/dev/";//new char[sizeof(SYS_DRIVER_PATH) + strlen(driver)];
//  sprintf(sysfs_dir, SYS_DRIVER_PATH "%s", driver);
  ...
  for (;;) {
    ...
    while ((ent = readdir(dir))) {
      //if (!strncmp("tty.usbserial", ent->d_name, strlen("tty.usbserial"))) {
         if (!strncmp("cu.usbserial", ent->d_name, strlen("cu.usbserial"))) {
